...
description: Create or update an M2 monthly KPI report as a Google Sheet workbook, with CSV fallback, from project-management evidence, the M2 monthly report example workbook structure, M2 project metrics, development plans, risk reports, staffing/status data, client feedback, or explicit manager facts. Use when preparing monthly M2 bonus/KPI reporting, checking which M2 obligations/KPIs can be supported, identifying missing evidence, or turning M2 monthly-report source data into a spreadsheet.
---
...
- M2 monthly KPI report Google Sheet workbook, with CSV fallback
...
3. Read `../qa-management-roles/references/m2-role/m2-role-basics.md`
   ...
   (blank-with-reason Template Consistency discipline).
...
6. Resolve the target workbook set before writing: the manager's own
   workbook under `20_M2_Project_Management`, and any external destination
   workbook explicitly provided by the user. External folder IDs and file IDs
   are runtime parameters; do not write real external IDs into this public
   repository.
7. Read the smallest relevant evidence set:
   - project metrics
...
8. Ask questions only for fields required to complete the requested report and not present in the evidence.
...
2. Use one living workbook per destination, not one new Google Sheet file
   per month. For a new reporting month, add a new sheet/tab named exactly
   `YYYY-MM` in each target workbook.
3. Update both configured copies in the same pass: the manager's own
   workbook in the QA Management workspace and the external destination
   workbook. If one write fails, report which copy failed and why; do not
   silently leave the two copies divergent.
4. For every row, fill only source-backed cells.
5. Leave unsupported cells blank.
6. Ask missing-data questions before finalizing when the requested report requires those cells.
7. Use `Да` only when there is direct evidence for the reporting period.
8. Use `Нет` only when the source explicitly says the case did not happen, or the user confirms it.
9. Leave `Completed (Да/Нет)` blank when the status is unknown.
10. Preserve the 2D CSV layout from `Templates\m2_monthly_report.csv`; do not convert it into a normalized table.
...
- Do not create a fresh external copy for every month. Copy only when a
  destination workbook does not exist yet or the user explicitly asks for a
  separate version.
