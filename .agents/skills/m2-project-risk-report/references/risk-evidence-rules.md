# M2 Project Risk Evidence Rules

Scope: input sources, evidence-handling rules, the risk-level dictionary, and risk-interpretation notes for `project_risk`.
...
- extracted source corpus under `G:\My Drive\QA_Management\90_Storage\_System\extracts\source\YYYY-MM-DD`
...
- Put source names or dated meetings in `evidence_log`, not in this Sheet — `project_risk` has no evidence column (see `risk-schema.md`'s Schema section).
...
- `Общий уровень риска` answers one specific question: does something concretely
  ...
  derived from (`90_Storage\Reference\Source_Documents\M2_project_development_plan`) never
