...
1. Run `scan_m1_events.py` (`--person <Name>` to scope it). It prints candidates and writes a bundle to `_System/reviews/open_questions/YYYY-MM-DD_m1.md`. It skips anything already logged (matched by a `scan:<kind>:<key>` tag in `Источник`).
