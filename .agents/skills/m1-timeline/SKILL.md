...
- workspace-wide `_m1_timeline` Google Sheet in `10_M1_People_Management`, with CSV fallback
...
2. **PR-cadence cross-check** — independently computes the expected next PR date from `_people_registry`'s `Дата трудоустройства`/`Дата последнего PR` (see `performance-review-rules.md`, "Deriving Expected Next PR Date": last PR + 6 months, or hire date + 3 months if no PR has happened yet). If there's no OKR Doc yet, this registry-computed date becomes the due date instead of "today" — so a person is tracked even before their first OKR Doc exists. If an OKR Doc *does* exist but its title date is more than ~2 weeks off from the registry-computed date, that mismatch is surfaced in `Комментарии` instead of silently trusting the Doc.
...
- If `_people_registry` has neither `Дата трудоустройства` nor `Дата последнего PR` for a person, the scan cannot compute an expected PR date — it falls back to "today" with a note that both fields are missing. Treat that as a data-gap candidate (ask for the hire date) rather than trusting the "today" placeholder as real.
