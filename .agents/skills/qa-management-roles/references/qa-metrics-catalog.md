...
verdicts. See `m2-role-rules.md`, Metrics Are Signals, Not Verdicts, for
...
- RCA (root cause analysis) is required before attributing production
  leakage to a person (see `m2-role-rules.md`, Production Bug Leakage
  Attribution)
...
(blank-with-reason when data isn't collected yet — see `m2-role-rules.md`,
...
- Prod leakage attributable after RCA — note: only once an RCA has
  ...
  person being the only QA on the project (see `m2-role-rules.md`,
