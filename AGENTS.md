...
| `visual-evidence-intake` | Common | Organizes raw screenshots dumped in `00_Inbox/_Visual_Drop` into a renamed/grouped bundle with a preserved original-to-new filename mapping and a recommended downstream path (Project Knowledge or M2 supporting context) - does not write any business document itself | `.agents/skills/visual-evidence-intake/SKILL.md` |
...
- `00_Inbox`: the only intake folder; recursively scanned, and empty means there are no unprocessed files. `00_Inbox/_Visual_Drop/` is a special subfolder for raw screenshot dumps (arbitrary filenames, plus an optional `visual_context.md`) - image files aren't picked up by `scan`, so they sit inert until organized via the `visual-evidence-intake` skill
